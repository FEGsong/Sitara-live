...
class FirestoreService {
  ...
  Future<void> updateProfile(String uid, {String? username, String? nickname, bool? profilePublic}) {
  ...
  Stream<List<Map<String, dynamic>>> allUsers() {
    return _users.orderBy('createdAt', descending: true).snapshots().map(
          (snap) => snap.docs.map((d) => {'uid': d.id, ...d.data() as Map<String, dynamic>}).toList(),
        );
  }
  ...
  Future<void> removeAdmin(String uid) => _users.doc(uid).update({'isAdmin': false});
