...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _approve(Map<String, dynamic> req) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('✅ ${req['coins']} coins added for ${req['phone']}')),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          title: const Text('Admin Panel'), backgroundColor: AppColors.bgDeep),
      body: ListView(
        ...
        children: [
          ...
          _box(
            title: 'Admins',
            subtitle:
                'These numbers trigger the "Admin watching" effect automatically whenever they join any live stream',
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(
                        child: TextField(
                            controller: _newAdminCtrl,
                            decoration: const InputDecoration(
                                hintText: '+92 3XX XXXXXXX'))),
                    const SizedBox(width: 8),
                    ElevatedButton(
                        onPressed: _addAdmin, child: const Text('Add')),
                  ],
                ),
          ...
          const Padding(
            ...
            child: Text('PENDING COIN REQUESTS',
                style: TextStyle(
                    fontSize: 12,
            ...
            builder: (context, snap) {
              ...
              return Column(
                children: requests.map((r) {
                  final method = kPayMethods.firstWhere(
                      (m) => m['id'] == r['method'],
                      orElse: () => {'name': r['method']});
                  return Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                            'Amount: ${r['price']} · Method: ${method['name']}\nReference: ${r['ref']}',
                            style: const TextStyle(
                                fontSize: 11.5,
                                color: AppColors.muted,
                                height: 1.5)),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                              ...
                              child:
                                  const Text('✅ Confirm Payment — Add Coins')),
                        ),
                      ],
                    ),
                  );
